/* x86-64 Linux ML-KEM benchmark harness using mlkem-native v1.2.0.
 * Run inside WSL2 Ubuntu (or any x86-64 Linux) beside the mlkem-native source.
 * Produces the identical 22-column CSV schema as the Android and RISC-V harnesses.
 *
 * Key differences from mlkem_android_benchmark.c:
 *   - Reads the actual CPU model from /proc/cpuinfo (not uname.machine).
 *   - Defaults to BENCHMARK_MEASUREMENT_TYPE="NATIVE_SOFTWARE".
 *   - Defaults to BENCHMARK_ARCHITECTURE="x86_64".
 *   - Compiler identification uses __GNUC__ / __clang__ macros.
 */
#define _GNU_SOURCE
...
/* Some Termux Android API headers omit this declaration; keep it for parity. */
...
#define BENCHMARK_OPTIMIZATION_FLAGS "-O3"
#endif
...
/* Identify compiler name and version. */
#ifdef __clang__
...
/* Read the first "model name" value from /proc/cpuinfo.
 * Returns the trimmed string, e.g. "AMD Ryzen 7 4800H with Radeon Graphics".
 * Falls back to "x86_64" if the file cannot be read. */
static void get_cpu_model(char *output, size_t size)
{
  FILE *f;
  char line[512];
  f = fopen("/proc/cpuinfo", "r");
  if (f == NULL) { snprintf(output, size, "x86_64"); return; }
    ...
    if (strncmp(line, "model name", 10) == 0) {
      ...
      if (colon != NULL) {
        ...
        while (len > 0 &&
               (start[len-1] == '\n' || start[len-1] == '\r' ||
                start[len-1] == ' ')) { len--; }
        snprintf(output, size, "%.*s", (int)len, start);
        fclose(f);
        return;
      }
    }
...
/* Required by mlkem-native randomized APIs. */
...
static long memory_bytes(void)
{
  ...
  return usage.ru_maxrss * 1024L; /* Linux reports KiB */
}
...
int main(int argc, char **argv)
{
  ...
  if (require_argument(argc, argv, "--iterations", &iteration_text) ||
    ...
    fprintf(stderr,
            "Usage: %s --iterations N --output FILE --environment NAME\n",
            argv[0]);
    return 2;
  }
  ...
  /* Collect OS info from uname; read actual CPU model from /proc/cpuinfo. */
  if (uname(&system_info) == 0) {
    snprintf(os_str, sizeof(os_str), "%s %s", system_info.sysname,
             system_info.release);
  } else {
    snprintf(os_str, sizeof(os_str), "Linux/WSL2");
  }
  ...
  /* Write CSV header — must match REQUIRED_COLUMNS in Python framework. */
  ...
  for (iteration = 1; iteration <= iterations; ++iteration) {
    ...
    write_row(file, experiment_id, run_id, environment, processor,
              ...
              rc == 0,
              rc == 0 ? "" : "ML-KEM key generation failed");

    /* Encapsulation — fresh keypair for setup, only enc is timed */
    rc = crypto_kem_keypair(pk, sk);
    if (rc == 0) {
      start = monotonic_ns();
    ...
    write_row(file, experiment_id, run_id, environment, processor,
              ...
              rc == 0,
              rc == 0 ? "" : "ML-KEM encapsulation setup or RNG failed");

    /* Decapsulation — only dec is timed; shared-secret verified */
    if (rc == 0) {
      start = monotonic_ns();
    ...
    write_row(file, experiment_id, run_id, environment, processor,
              ...
              rc == 0,
              rc == 0 ? "" : "ML-KEM decapsulation or shared-secret verification failed");
  }

  if (fclose(file) != 0) { perror("close output"); return 2; }
  fprintf(stdout,
          "Wrote %d rows for %s to %s (commit %s)\n",
