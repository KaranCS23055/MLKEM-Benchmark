/* 32-bit x86 Linux ML-KEM benchmark harness using mlkem-native v1.2.0.
 * Compiled with gcc -m32 inside WSL2 Ubuntu.
 * Produces the identical 22-column CSV schema.
 */
#define _GNU_SOURCE
...
#define BENCHMARK_OPTIMIZATION_FLAGS "-m32 -O3"
#endif
...
static void get_cpu_model(char *output, size_t size)
{
  ...
  if (f == NULL) { snprintf(output, size, "x86 (32-bit)"); return; }
    ...
    if (strncmp(line, "model name", 10) == 0) {
      ...
      if (colon != NULL) {
        ...
        while (len > 0 &&
               (start[len-1] == '\n' || start[len-1] == '\r' ||
                start[len-1] == ' ')) { len--; }
        snprintf(output, size, "%.*s (32-bit mode)", (int)len, start);
        fclose(f);
        return;
      }
    }
  ...
  fclose(f);
  snprintf(output, size, "x86 (32-bit)");
}
...
int main(int argc, char **argv)
{
  ...
  if (uname(&system_info) == 0) {
    snprintf(os_str, sizeof(os_str), "%s %s (i686)", system_info.sysname, system_info.release);
  } else {
    snprintf(os_str, sizeof(os_str), "Linux 32-bit");
  }
  ...
  fclose(file);
  fprintf(stdout, "Wrote %d 32-bit x86 rows for %s to %s\n", iterations * 3, variant, output_path);
  return 0;
}
