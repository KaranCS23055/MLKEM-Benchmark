#!/usr/bin/env python3
"""
Automated ML-KEM Benchmarking Runner for ESP8266
Compiles, flashes, and captures standardized CSV output from ESP8266 over serial UART.
"""
...
SKETCH_DIRS = {
    "512": os.path.join(BASE_DIR, "mlkem_esp8266_bench"),
    "768": os.path.join(BASE_DIR, "mlkem_esp8266_bench_768"),
    "1024": os.path.join(BASE_DIR, "mlkem_esp8266_bench_1024"),
}

def find_default_port():
    ports = [p.device for p in serial.tools.list_ports.comports() if "USB" in p.description or "CH340" in p.description or "CP210" in p.description]
    if ports:
        return ports[0]
    all_ports = [p.device for p in serial.tools.list_ports.comports()]
    return all_ports[0] if all_ports else "COM3"
...
def capture_benchmark(port, baudrate, variant, timeout_sec=90):
    ...
    # Toggle DTR/RTS to reset the ESP8266 cleanly
    ser.setDTR(False)
    ser.setRTS(True)
    time.sleep(0.1)
    ser.setRTS(False)
    ...
    out_filename = f"esp8266_xtensa_lx106_arduino_mlkem_{variant}_{ts}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Run ML-KEM Benchmark on ESP8266")
    parser.add_argument("--variant", choices=["512", "768", "1024", "all"], default="768",
    ...
    variants = ["768", "1024"] if args.variant == "all" else [args.variant]
    
    for v in variants:
        print(f"\n{'='*60}")
        print(f"  BENCHMARKING ML-KEM-{v} ON ESP8266 ({args.port})")
        print(f"{'='*60}")
